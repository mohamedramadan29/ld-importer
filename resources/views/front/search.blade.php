...
@section('content')
    ...
    <header class="page-hero" style="min-height: 40vh; height: auto; padding: 180px 0 80px 0;">
        <div class="container" data-aos="zoom-out" data-aos-duration="1200">
            ...
            <p class="hero-desc">
                {{ $query ? ($searchHero['description'] ?? 'Results for') . ' "' . $query . '"' : $searchHero['description'] ?? 'Search our furniture collection' }}
            </p>
        </div>
    </header>
            ...
            <div class="row g-3 justify-content-center align-items-center">
                <div class="col-md-5">
                    <div class="position-relative">
                        <i class="fa-solid fa-magnifying-glass position-absolute"
                            style="left: 20px; top: 50%; transform: translateY(-50%); color: #999; pointer-events: none;"></i>
                        <input type="text" name="q" class="form-control search-main-input"
                            placeholder="חיפוש ריהוט, שולחנות, מראות..." value="{{ $query }}"
                ...
                <div class="col-md-2">
                    <select name="sort" class="form-select search-select-input">
                        <option value="latest" {{ $sort == 'latest' ? 'selected' : '' }}> החדשים ביותר </option>
                        <option value="price_low" {{ $sort == 'price_low' ? 'selected' : '' }}>מחיר: מהנמוך לגבוה</option>
                        <option value="price_high" {{ $sort == 'price_high' ? 'selected' : '' }}>מחיר: מהגבוה לנמוך</option>
                        <option value="name" {{ $sort == 'name' ? 'selected' : '' }}>שם א-ת</option>
                    </select>
                </div>
                <div class="col-md-2">
                    <button type="submit" class="btn search-btn-submit">
                        <i class="fa-solid fa-magnifying-glass me-2"></i> חיפוש
                    </button>
                </div>
            </div>
        </form>
    ...
    <section class="container mt-4">
        <div class="d-flex justify-content-between align-items-center">
            <p class="text-muted mb-0">
                <span class="fw-bold">{{ $products->count() }}</span> מוצרים נמצאו
                @if ($query)
                    עבור "<span class="fw-bold">{{ $query }}</span>"
                @endif
            </p>
            <div class="view-mode-icons d-none d-sm-flex">
                <i class="fa-solid fa-border-all active" id="gridViewBtn" title="תצוגת רשת"></i>
                <i class="fa-solid fa-list" id="listViewBtn" title="תצוגת רשימה"></i>
            </div>
        </div>
    </section>
    ...
    <main class="container mt-3 mb-5">
        @if ($products->count() > 0)
            <div class="row g-3 g-md-4 catalog-main-wrapper" id="productsCatalogWrapper">
                @foreach ($products as $product)
                    <div class="col-6 col-md-3 product-catalog-item" data-aos="fade-up"
                        ...
                        <a href="{{ route('front.product', $product->slug) }}" class="text-decoration-none">
                            <div class="catalog-img-box">
                                <img src="{{ $product->mainImage ? asset('assets/uploads/products/' . $product->mainImage->image) : 'https://via.placeholder.com/600x400' }}"
                                    alt="{{ $product->name }}">
                                <button type="button"
                                    class="favorite-btn {{ in_array($product->id, session()->get('favorites', [])) ? 'active' : '' }}"
                                    data-id="{{ $product->id }}"
                                    onclick="event.preventDefault(); event.stopPropagation(); toggleFavorite(this);">
                                    <i
                                        class="fa{{ in_array($product->id, session()->get('favorites', [])) ? 's' : 'r' }} fa-heart"></i>
                                </button>
                                @if ($product->has_discount)
                                    <span class="badge badge-danger position-absolute"
                                        style="top:10px;left:10px;background-color:var(--premium-red);">מבצע</span>
                                @endif
                                <button
                                    class="quick-add-overlay-btn">{{ $labels['view_details'] ?? 'VIEW DETAILS +' }}</button>
                            </div>
                            <div class="catalog-meta">
                                ...
                                <div class="catalog-price">
                                    @if ($product->has_discount)
                                        <span
                                            style="text-decoration:line-through;color:#999;font-size:12px;">₪{{ number_format($product->price, 0) }}</span>
                                        <span
                                            class="text-danger ms-1">₪{{ number_format($product->discount_price, 0) }}</span>
                                    @else
        ...
        @else
            <div class="text-center py-5">
                ...
                <p class="text-muted">
                    {{ $searchEmpty['description'] ?? 'Try different keywords or browse our categories.' }}</p>
                <a href="{{ route('front.category') }}"
                    class="btn btn-dark mt-3">{{ $searchEmpty['button_text'] ?? 'Browse All Products' }}</a>
            </div>
        @endif
    </main>
